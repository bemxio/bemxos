...
void isr_install() {
    // can't do this with a loop because it needs the address of the function names
...
void isr_handler(registers_t registers) {
    char *exception = exceptions[registers.interrupt];
    char code[3];
...
void register_interrupt_handler(u8 value, isr_t handler) {
    interrupt_handlers[value] = handler;
}

void irq_handler(registers_t registers) {
    // after every interrupt we need to send an EOI to the PICs
    // or they will not send another interrupt again
    u32 interrupt = registers.interrupt;
